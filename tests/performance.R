...
measure.call_time <- function(n=1000000, y=10000)
{
  data <- Returns(rnorm(y))
  
  method1 <- function(x) { for (i in n) s3sd(x) }
  method2 <- function(x) { for (i in n) lrsd(x) }
  method3 <- function(x) { for (i in n) lrsd1(x) }
